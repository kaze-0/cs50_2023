...
void tabulate(void)
{
    for (int c = 0; c < candidate_count; c++)
    {
        if (candidates[c].eliminated)
            continue;
        candidates[c].votes = 0; // init

        for (int v = 0; v < voter_count; c++)
        {
            if (preferences[v][c] == 0)
                candidates[c].votes++; // if rank0 then calc
        }
    }
...
bool print_winner(void)
{
    ...
    for (int c = 1; c < candidate_count; c++)
    {
        if (candidates[c].eliminated)
            continue;
        if (candidates[c].votes > winner.votes)
        {
            winner = candidates[c];
        }
    }
    if (winner.votes > voter_count * 1.0 / 2)
    {
        printf("%s\n", winner.name);
        return true;
    }
        ...
        if (candidates[c].votes == min)
        {
            ...
            for (int v = 0; v < voter_count; v++)
            {
                for (int e = 0; e < candidate_count; e++)
                    ...
                    if (preferences[v][e] > preferences[v][c])
                        preferences[v][e]--; // rank up
            }
        }
    }
    return;
